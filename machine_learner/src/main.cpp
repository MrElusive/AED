...
public:
	...
	ArgParser ( char* argv[], int argc )
	{
		...
		discretize = false;
        //These values are defined in backpropagation.h
        numHiddenLayers = DEFAULT_NUM_HIDDEN_LAYERS;
        hiddenNodesPerLayer = DEFAULT_NUM_HIDDEN_NODES;
		...
		for ( int i = 1; i < argc; i++ )
		{
			...
			else if ( strcmp ( argv[i], "-E" ) == 0 )
			{
                ...
                hiddenNodesPerLayer = atoi ( argv[++i] );
            else if ( strcmp ( argv[i], "-l" ) == 0 )
                numHiddenLayers = atoi ( argv[++i] );
			else
    ...
    int getHiddenNodesPerLayer(){return hiddenNodesPerLayer;}
    int getK(){return k;}
};
...
SupervisedLearner* getLearner(string model, Rand& r, int layers, int nodes,int k)
{
	if (model.compare("baseline") == 0)
        ...
        return new BackPropagationLearner(r,layers, nodes);
...
void doit(ArgParser& parser)
{
    ...
    //get special args needed for some models
    int layers = parser.getNumHiddenLayers();
    int nodes = parser.getHiddenNodesPerLayer();
    int k = parser.getK();
	SupervisedLearner* learner = getLearner ( model, r, layers, nodes , k);
