...
Matrix& Matrix::operator=(Matrix& source)
{
    if(this == &source)
        return *this;
    m_attr_name = source.m_attr_name;
	m_str_to_enum = source.m_str_to_enum;
	m_enum_to_str = source.m_enum_to_str;
    size_t rows = source.rows();
    for(size_t r=0;r<rows;r++)
    {
        std::vector<double> row = source.row(r);
        m_data.push_back(row);
    }
    return *this;
}

std::vector<double> Matrix::getValues(size_t attr)
{
    std::vector<double> vals;
    for(size_t i=0;i<m_data.size();i++)
    {
        double val = m_data[i][attr];
...
Matrix* Matrix::createCopyForBinaryPerceptron(int target)
{
...
{
    Matrix* pmat = (Matrix*)(&mat); //This is to get around const
    os<<std::endl;
    ...
    for(size_t c=0;c<mat.cols();c++)
        os<<"Attribute["<<c<<"]:"<<mat.attrName(c)<<std::endl;
    return os;
}
