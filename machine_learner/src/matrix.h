...
public:
    ...
    //assigns a complete copy of the matrix (not like the copy constructor)
    Matrix& operator=(Matrix& other);

    /*
        This function is used for label matrices for perceptrons with multiple
        output catagories.
        It returns a copy of the matrix except that every cell with the value 
        specified by the target parameter will be changed to 1, and every other
        value will be changed to 0

        Remember to delete this matrix when you are done
    */
    Matrix* createCopyForBinaryPerceptron(int target);
	...
	std::string attrName(size_t col) const { return m_attr_name[col]; }

	// Returns the name of the specified value
	std::string& attrValue(size_t attr, size_t val){return m_enum_to_str[attr][val];}

    //removes the attribute from the metadata
    void removeAttr(size_t attr){m_attr_name.erase(m_attr_name.begin()+attr);
                                m_enum_to_str.erase(m_enum_to_str.begin()+attr);}
    ...
    //Note that if it you remove rows or something, this count will not go down.
    //For the actual current value count to getValues(attr).size()
	size_t valueCount(size_t attr) { return m_enum_to_str[attr].size(); }

    // Returns a vector of all of the unique values for the given attribute
    std::vector<double> getValues(size_t attr);
...
//Overload the ostream << operator for Matrix
